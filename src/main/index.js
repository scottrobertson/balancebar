...
import { app, Tray, ipcMain, nativeTheme, Menu, nativeImage } from "electron";
...
app.on("ready", () => {
  ...
  mb.on("ready", () => {
    ...
    const filter = { urls: ["http://localhost/oauth*"] };
    webRequest.onBeforeRequest(filter, async ({ url }) => {
      await mb.window.loadURL(winURL);
      mb.window.setAlwaysOnTop(false);
      mb.window.webContents.send("handle-oauth", url);
    });
  });
});
